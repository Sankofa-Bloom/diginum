...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    try {
      // Call Swychr API to check payment status
      ...
      if (swychrResponse.data && swychrResponse.data.status === 200) {
        return {
          ...
          body: JSON.stringify({
            success: true,
            data: {
              status: swychrResponse.data.data?.status || 'pending',
              amount: swychrResponse.data.data?.amount || 0,
              currency: swychrResponse.data.data?.currency || 'USD',
              transaction_id: transaction_id
            },
            status: swychrResponse.data.status,
            message: swychrResponse.data.message || 'Payment status retrieved successfully'
          })
        };
      } else {
        return {
          ...
          body: JSON.stringify({
            success: false,
            message: swychrResponse.data?.message || 'Failed to get payment status',
