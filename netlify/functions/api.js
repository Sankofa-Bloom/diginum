...
const getCountryCode = (countryName) => {
  ...
  return countryCodes[countryName] || '+1'; // Default to +1 if not found
};
...
exports.handler = async (event, context) => {
  ...
  try {
    ...
    if (endpoint === 'countries' && httpMethod === 'GET') {
      try {
        console.log('Fetching countries from SMS provider...');
        
        // Check if SMS API key is available
        const smsApiKey = process.env.SMS_API_KEY || process.env.VITE_SMS_API_KEY;
        if (!smsApiKey) {
          console.log('SMS API key not found, using fallback countries');
          throw new Error('SMS API key not configured');
        }

        // Call SMS provider API to get countries
        const smsApiUrl = 'https://api.sms-activate.org/stubs/handler_api.php';
        const response = await fetch(`${smsApiUrl}?api_key=${smsApiKey}&action=getCountryAndOperators`);
        ...
        console.log('SMS API response:', countriesData);
        ...
        console.log('Transformed countries:', countries.length, 'countries loaded');
        ...
        console.error('Error fetching countries from SMS provider:', error);
